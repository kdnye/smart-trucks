...
| **EDGE_INGEST_KEY rename** | deferred | Decouple the Pi's ingest auth secret from the misleading `MOTIVE_API_KEY` name. | Not required for base model. | Add a dedicated secret on the ingest function + a Balena var of the same name; accept the old `MOTIVE_API_KEY` during transition. |
