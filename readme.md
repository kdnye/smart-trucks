...
1.  **`ble-sensor`**: Scans for Govee BLE advertisements. Extracts temperature, humidity, and battery health.
2.  **`gps-telematics`**: Interchanges with the BerryGPS-IMU V4 via UART. Processes NMEA sentences and evaluates geofencing logic.
3.  **`sync-service`**: Manages the data queue. Synchronizes logs with the cloud dashboard via webhooks when connectivity is available.
